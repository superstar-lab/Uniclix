...
import {
  startSetChannels,
  startAddFacebookChannel,
  startAddLinkedinChannel,
  startAddPinterestChannel,
...
class ConnectAccounts extends React.Component {
  state = {
    ...
    billingPeriod:
      getParameterByName("period", this.props.location.search) || "annually",
  ...
  onTwitterSuccess = (response) => {
    ...
    try {
      response.json().then((body) => {
        this.props
          .startAddTwitterChannel(body.oauth_token, body.oauth_token_secret)
          .then(() => {
          ...
          .catch((error) => {
            ...
            if (error.response.status === 409) {
              Modal({
                ...
                title: "Error",
                content:
                  "This account is currently being used by other Uniclix users, please contact our helpdesk support for additional details",
              });
              ...
              Modal({
                ...
                title: "Error",
                content:
                  "You reached the limit of accounts for your current plan. Please upgrade to continue adding them.",
                okText: "Upgrade",
  ...
  onFacebookSuccess = (response) => {
    try {
      ...
      if (response) {
        this.props
          .startAddFacebookChannel(response.accessToken)
          .then(() => {
            this.setState(() => ({ loading: true }));
            getAccounts()
              .then((response) => {
          ...
          .catch((error) => {
            ...
            if (error.response.status === 409) {
              ...
              Modal({
                ...
                title: "Error",
                content:
                  "You reached the limit of requests to Facebook's API, please try again in 20 minutes",
              });
            } else {
              Modal({
                type: "error",
  ...
  onLinkedInSuccess = (response) => {
    try {
      this.setState(() => ({ loading: true }));
      this.props
        .startAddLinkedinChannel(response.accessToken)
        .then(() => {
  ...
  remove = (id) => {
    ...
    return destroyChannel(id)
      .then((response) => {
        this.setState(() => ({ loading: false }));
        this.props.startSetChannels().then((response) => {
  ...
  renderTypeaccounts(param) {
    return (
      <div className="channel-buttons">
        <ChannelItems
          channels={[param]}
          setAction={() => this.remove(param.id)}
        />
      ...
      case "twitter":
        return (
          <div className="channel-buttons">
            ...
            >
              <i className="fab fa-twitter"></i> Add Another Account
            </button>
          </div>
        );
      case "facebook":
  ...
  getAccountsForModal = () => {
    ...
    const filteredAccounts = bussinesPages.filter(
      (page) =>
        facebookChannels.findIndex(
          (fb) => fb.details.original_id === page.id
        ) === -1
    );
  ...
  getFacebookLabel = () => {
    ...
    channels
      ...
      .map((item) => {
        if (item.details.account_type === "profile")
          fbAccounts.profiles.push(item);
    ...
    if (fbAccounts.pages.length)
      subTitle.push(`${fbAccounts.pages.length} pages`);
    if (fbAccounts.groups.length)
      subTitle.push(`${fbAccounts.groups.length} groups`);
    if (fbAccounts.profiles.length)
      subTitle.push(`${fbAccounts.profiles.length} profiles`);
    ...
    return (
      <div className="btn-label">
        ...
        {!!subTitle.length && (
          <div className="sub-title">{subTitle.join(" | ")}</div>
        )}
      </div>
    );
  };
  ...
  getTwitterLabel = () => {
    ...
    return (
      <div className="btn-label">
        ...
        {!!subTitle && (
          <div className="sub-title">{`${subTitle} profiles`}</div>
        )}
      </div>
    );
  };
  ...
  getLinkedinLabel = () => {
    ...
    channels
      ...
      .map((item) => {
        if (item.details.account_type === "profile")
          liAccounts.profiles.push(item);
        if (item.details.account_type === "page") liAccounts.pages.push(item);
      });

    if (liAccounts.pages.length)
      subTitle.push(`${liAccounts.pages.length} pages`);
    if (liAccounts.profiles.length)
      subTitle.push(`${liAccounts.profiles.length} profiles`);
          ...
          {!!accountsModal && (
            ...
            >
              ...
              <div style={{ float: "right" }}>
                <button
                  onClick={() => this.setState({ accountsModal: false })}
                  className="cancelBtn"
                >
                  No
                </button>
                <a href="/settings/billing" className="cancelBtn1">
                  Yes
                </a>
              </div>
            </ReactModal>
...
const mapDispatchToProps = (dispatch) => ({
  ...
  startAddTwitterChannel: (token, secret) =>
    dispatch(startAddTwitterChannel(token, secret)),
  startAddLinkedinChannel: (token) => dispatch(startAddLinkedinChannel(token)),
  startAddPinterestChannel: (token) =>
    dispatch(startAddPinterestChannel(token)),

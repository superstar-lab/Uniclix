...
class SelectChannelsModal extends React.Component {
  ...
  componentDidMount() {
    ...
    const twitterChannels = channelSelector(channels, {
      selected: undefined,
      provider: "twitter",
    });
    const facebookChannels = channelSelector(channels, {
      selected: undefined,
      provider: "facebook",
    });
    const linkedinChannels = channelSelector(channels, {
      selected: undefined,
      provider: "linkedin",
    });
    const pinterestChannels = channelSelector(channels, {
      selected: undefined,
      provider: "pinterest",
    });
    ...
    return (
      <div className="modal-content main-modal-style">
        <div className="modal-header-container">
          ...
          <button
            className="default-white-btn"
            onClick={this.onAddAccountsClick}
          >
            <span className="cus-plus-icon">+</span> Add accounts
          </button>
        </div>
          ...
          <div className="modal-menu col-md-4">
            <ul>
              {!!twitterChannels.length && (
                <li
                  onClick={this.toggleTwitterSelect}
                  className={this.state.twitterSelect && "selected"}
                >
                  <i className="fab fa-twitter twitter_color"> </i>{" "}
                  <p>Twitter</p>
                </li>
              )}
              ...
              {!!facebookChannels.length && (
                <li
                  onClick={this.toggleFacebookSelect}
                  className={this.state.facebookSelect && "selected"}
                >
              ...
              {!!linkedinChannels.length && (
                <li
                  onClick={this.toggleLinkedinSelect}
                  className={this.state.linkedinSelect && "selected"}
                >
              ...
              {!!pinterestChannels.length && (
                <li
                  onClick={this.togglePinterestSelect}
                  className={this.state.pinterestSelect && "selected"}
                >
                  <i className="fab fa-pinterest pinterest_color"> </i>{" "}
                  Pinterest
                </li>
              )}
            </ul>
            ...
            {!!twitterChannels.length &&
              ...
              twitterChannels.map((channel) => (
                ...
                >
                  <label
                    key={channel.id}
                    className="channel-item selection-container"
                  >
                    <input
                      type={twitterSelectType}
                      onChange={() => onChange(channel)}
                      defaultChecked={channel.selected ? "checked" : ""}
                      name="twitter_channel"
                    />
                    <span
                      className={`checkmark ${
                        twitterSelectType == "radio" ? "round" : ""
                      }`}
                    ></span>
                    <img

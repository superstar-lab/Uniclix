...
class AnalyticsLanding extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      selectedAccount: accountSelectorOptions[0]
        ? accountSelectorOptions[0].id
        : 0,
      selectedSocialMedia,
    };
  }
  ...
  componentDidMount() {
    ...
    if (this.state.selectedSocialMedia === "linkedin") {
      notification.open({
        message: "Heads up!",
        description:
          "We don't support LinkedIn Analytics currently. You can still use Facebook and Twitter though!",
        duration: 10,
      ...
      if (this.socialMediasSelectorOptions.length) {
        const socialMediaToRedirectIndex =
          this.socialMediasSelectorOptions.findIndex((sm) => sm !== "linkedin");
        if (socialMediaToRedirectIndex !== -1) {
          this.props.history.push(
            `/analytics/${this.socialMediasSelectorOptions[socialMediaToRedirectIndex]}`
          );
        } else {
  ...
  getAccountSelectorOptions = (selectedSocialMedia) => {
    ...
    let options = allChannels.filter(
      (account) => account.type === selectedSocialMedia
    );
        ...
        {!this.state.forbidden && (
          <React.Fragment>
              ...
              <div className="section-header__first-row">
                ...
                <div className="dropdown-selectors">
                  ...
                  <AccountSelector
                    ...
                    accounts={this.getAccountSelectorOptions(
                      selectedSocialMedia
                    )}
                  />
                </div>
            ...
            <AnalyticsContext.Provider
              value={{ setForbidden: this.setForbidden }}
            >

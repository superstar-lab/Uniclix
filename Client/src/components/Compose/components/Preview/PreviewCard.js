...
class PreviewCard extends React.Component {
  ...
  prepareText = () => {
    ...
    for (const match of matches) {
      const link = !!match[0].match(/http:\/\/|https:\/\//)
        ? match[0]
        : `http://${match[0]}`;
      text = text.replace(
        match[0],
        `<a href="${link}" target="_blank">${match[0]}</a>`
      );
    }
  ...
  render() {
    const { avatar, name, subTitle, pictures, socialMedia, videos } =
      this.props;
    ...
    return (
      <div className={`preview-card-container ${socialMedia}`}>
        ...
        {!!videos.length && (
          <div className="video">
            {videos.map((vid) => (
              <video controls loop src={`${vid}#t=0.1`} />
            ))}
          </div>
        )}
      </div>

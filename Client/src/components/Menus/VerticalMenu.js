...
class ProfileInfo extends React.Component {
  ...
  activeChannels = (channels) => {
    return channels;
    let msDiff =
      new Date(this.props.trialEnded).getTime() - new Date().getTime();
    if (Math.floor(msDiff / (1000 * 60 * 60 * 24)) >= 0) {
    } else {
      let activeChannels = channels.filter((channel) => {
        if (channel.details.paid == 1) return channel;
      });
  ...
  render() {
    ...
    return (
      <div>
        <div
          className="profile-info selected-profile"
        ...
        >
          <span className="pull-left profile-img-container">
            ...
            />
            <i
              className={`fab fa-${selectedChannel.type} ${selectedChannel.type}_bg smallIcon`}
            ></i>
          </span>
          <div>
            {!!selectedChannel.username ? (
              ...
              <p className="profile-name" title={selectedChannel.name}>
...
const ProfileSelectionDropDown = ({ channels, selectChannel, isOpen }) => (
  <div
    className={`channel-selection-menu select-channel ${
      isOpen ? "is-open" : ""
    }`}
    ...
    style={
      isOpen && channels.length ? { height: 85.5 * (channels.length + 1) } : {}
    }
  >
    <div>
      {!!channels.length &&
      ...
      <NavLink
        ...
        className="add-profile channel-container"
      >
  ...
  render() {
    ...
    return (
      <ul className="v-menu-links clear-both">
        {menuItems.map((item) => (
          <li key={item.id}>
            <NavLink className="links" to={item.uri}>

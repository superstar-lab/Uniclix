...
class Middleware extends React.Component {
  ...
  componentDidMount() {
    ...
    if (
      (this.state.plan || this.state.addon) &&
      !!profile &&
      !profile.subscription.activeSubscription &&
      !profile.addon.activeAddon
    ) {
  ...
  componentDidUpdate(prevProps) {
    if (
      ...
      if (
        ...
        !this.props.profile.addon.activeAddon &&
        !this.state.addonTrial
      ) {
        this.props.setMiddleware("billing");
        return;
      } else {
  ...
  render() {
    ...
    const { loading, allPlans, addon, addonTrial, addAccounts, bussinesModal } =
      this.state;
    let planParam = getParameterByName("plan", this.props.location.search);
    let planData = allPlans.filter(
      (plan) => plan["Name"].toLowerCase() === planParam
    );
    ...
    if (planData) {
      planName =
        this.state.billingPeriod === "annually"
          ? planData["Name"].toLowerCase() + "_annual"
          : planData["Name"].toLowerCase();
    }
      ...
      <div className={`login-container ${middleware}`}>
        ...
        <div className="col-md-7 col-xs-12 text-center">
          <div className="col-xs-12 text-center">
            ...
            />
            {middleware !== "channels" && middleware !== "billing" && (
              <Loader />
            )}
            ...
            {middleware == "channels" && (
              <div className="box channels-box">
                <div>
                  ...
                  {channels.length > 0 ? (
                    <button className="magento-btn mt50" onClick={this.setRole}>
          ...
          {middleware == "billing" && !!planData ? (
            <div className="box billing channels-box">
              ...
              <div className="plan-box col-md-6 col-xs-12">
                <div
                  className={`billingPeriodSelection col-md-12 ${
                    this.state.billingPeriod === "annually" && "selected"
                  }`}
                >
                  <label className="custom-radio-container">
                    ...
                    <input
                      ...
                      onChange={() => this.setBillingPeriod("annually")}
                    />
                    <span className="checkmark"></span>
                  </label>

                  <p>
                    ${parseFloat(planData["Annual Billing"] / 12).toFixed(1)} /
                    month
                  </p>
                  <p>
                    Billing annually for $
                    {parseFloat(planData["Annual Billing"]).toFixed(1)}
                  </p>
                </div>
              </div>
              <div className="plan-box col-md-6 col-xs-12">
                <div
                  className={`billingPeriodSelection col-md-12 ${
                    this.state.billingPeriod === "monthly" && "selected"
                  }`}
                >
                  <label className="custom-radio-container">
                    Monthly
                  ...
                  <p>
                    Billing monthly for $
                    {parseFloat(planData["Monthly"]).toFixed(1)}
                  </p>
                </div>
              </div>
              ...
              {!!planData && (
                <Checkout
                  ...
                  text=""
                >
                  <button className="magento-btn mt50">
                    Proceed to Checkout
                  </button>
                </Checkout>
              )}
            ...
            <div className="box billing channels-box">
              ...
              <div className="plan-box col-md-12 col-xs-12">
                ...
                >
                  <label className="custom-radio-container">
                    {`${
                      !!addonTrial ? "Free trial period of 3 days" : "Monthly"
                    }`}
                  ...
                  <p>{`${
                    !!addonTrial ? "3 days free trial" : "$10.0 / month"
                  }`}</p>
                  <p>
                    Billing monthly for{" "}
                    {!!addonTrial ? "$0.0 (3 days free trial)" : "$10.0"}
                  </p>
                </div>
              </div>

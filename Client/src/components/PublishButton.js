...
class PublishButton extends React.Component {
  ...
  componentDidMount() {
    if (this.props.post) {
      const postDate =
        (this.props.post ? this.props.post.type : "store") === "edit"
  ...
  setScheduledLabel = () => {
    ...
    if (this.state.canSchedule && this.state.publishState.value === "date") {
      scheduledLabel = `Scheduled: ${moment(this.state.publishDateTime).format(
        "DD MMMM YYYY hh:mmA"
      )}`;
    }
  ...
  setPublishTimestamp = () => {
    ...
    const formatted24HTime = moment(
      `${time.hour}:${time.minutes} ${time.time}`,
      "hh:mm a"
    ).format("HH:mm");
              ...
              {!this.state.showCalendar ? (
                <div className="tooltip-menu">
                  ...
                  >
                    ...
                    {!(
                      this.state.canSchedule &&
                      this.state.publishState.value === "date"
                    ) ? (
                      <p>Schedule at a specific time</p>
                    ) : (
                      <p className="schedule-info">
                        {moment(this.state.publishDateTime).format(
                          "DD MMMM YYYY hh:mmA"
                        )}
                      </p>
                    )}
                      ...
                      <div>
                        <div className="uc-calendar-time-picker">
                          <select
                            ...
                            value={this.state.calendarData.time.hour}
                            className="hours"
                          >
                            {hours.map((hour) => (
                          ...
                          <select
                            ...
                            value={this.state.calendarData.time.minutes}
                            className="minutes"
                          >
                            {minutes.map((minute) => (
                          ...
                          <select
                            ...
                            value={this.state.calendarData.time.time}
                            className="dayTime"
                          >
                            {dayTime.map((time) => (
                        ...
                        <div
                          onClick={() => {
                            if (this.state.canSchedule) {
                              this.setPublishState(
                                { name: "Custom Time", value: "date" },
                                close
                              );
                            }
                          }}
                          className={`btn naked-button date-btn ${
                            !this.state.canSchedule ? "disabled" : ""
                          }`}
                        >
                          Done
                        </div>

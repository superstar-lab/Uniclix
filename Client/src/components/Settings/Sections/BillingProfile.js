...
import { getCookie, capitalize } from "../../../utils/helpers";
...
class BillingProfile extends React.Component {
  constructor(props) {
    ...
    this.state = {
      allPlans: [],
      error:
        "Please delete some accounts to correspond to the limits of your new plan.",
      redirect: "/accounts",
      billingPeriod: this.props.profile.subscription.annual
        ? "annually"
        : "monthly",
      planChange: false,
      planCancel: false,
  ...
  componentDidMount() {
    ...
    getPlanData().then((response) => {
      this.setState({
        ...
        roleBilling: cookieParts
          ? cookieParts[0]
          : this.props.profile.role.name,
        ...
        if (error.response.status === 432) {
          FunctionModal({
            ...
            content: (
              <div>
                <div>
                  Please delete social media accounts to downgrade your plan.
                </div>
              </div>
            ),
          ...
          FunctionModal({
            type: "confirm",
            title:
              "You are over the limit of team members permitted for this plan",
            content: (
              <div>
  ...
  setBillingPeriod = () => {
    this.setState(() => ({
      billingPeriod:
        this.state.billingPeriod === "annually" ? "monthly" : "annually",
    }));
  };
      ...
      (status, response) => {
        response.plan =
          this.state.billingPeriod === "annually"
            ? this.state.planName + "_annual"
            : this.state.planName;
        response.trialDays = 0;
        response.created = new Date().getTime();
        response.subType = "main";
        response.couponCode =
          this.state.billingPeriod === "annually" ? this.state.couponCode : "";
        if (status === 200) {
  ...
  navigateToCongrats = (plan) => {
    this.props.history.push(
      `/settings/billing/thank-you-${plan.toLowerCase()}`
    );
  };
      ...
      <div>
        ...
        >
          ...
          <div style={{ float: "right" }}>
            <button
              onClick={() => this.setState({ showFreeTrialEndedModal: false })}
              className="cancelBtn"
            >
              OK
            </button>
          ...
          <div>
            {!!planConfirm && (
              <Modal
                ...
                isOpen={!!planConfirm}
              >
                <div className="modal-title">{`You can order ${capitalize(
                  planConfirm
                )}`}</div>
                <div className="modal-contents">{`Do you wish to proceed with this confirm order?`}</div>
                <div style={{ float: "right" }}>
                  <button
                    onClick={() => this.setPlanConfirm(false)}
                    className="cancelBtn"
                  >
                    No
                  </button>
                  <button
                    onClick={() => {
                      this.setPlanConfirm(false);
                      this.onPlanClick(planConfirm);
                    }}
                    className="cancelBtn"
                  >
                    Yes
            ...
            {!!planChange && (
              <Modal
                ...
                isOpen={!!planChange}
              >
                <div className="modal-title">{`Do you want to change your subscription to the ${capitalize(
                  planChange
                )} plan?`}</div>
                <div style={{ float: "right" }}>
                  <button
                    onClick={() => this.setPlanChange(false)}
                    className="cancelBtn"
                  >
                    No
                  </button>
            ...
            {!!planResume && (
              ...
              >
                ...
                <div style={{ float: "right" }}>
                  <button
                    onClick={() => this.setPlanResume(false)}
                    className="cancelBtn"
                  >
                    No
                  </button>
            ...
            {!!planCancel && (
              ...
              >
                ...
                <div className="spacing-left">
                  <b>{`Thank you for giving Uniclix a try`}</b>
                </div>
                <br />
                <div className="spacing-left">
                  <b>{`Why are you canceling your account?`}</b>
                </div>
                ...
                <div style={{ float: "right" }}>
                  <button
                    onClick={() => this.setPlanCancel(false)}
                    className="cancelBtn"
                  >
                    Cancel
                  </button>
            ...
            {allPlans.length > 0 ? (
              <div className="container billing-top">
                {freeTrialEnded && (
                  <div className="free-trial-end-msg">
                    <strong>You don't have an active subscription</strong>
                    <span>
                      To access your project again, please subscribe. Otherwise,
                      ...
                      <div key={index} className="col-4 col-md-4 col-sm-12">
                        ...
                        >
                          <div className="card-body">
                            <h6 className="card-selected text-center">
                              {plan["Name"].toLowerCase() == roleBilling
                                ? "Current plan"
                                : ""}
                            </h6>
                            ...
                            <ul className="fa-ul ">
                              <li>
                                ...
                                {plan["Social Accounts"]} social accounts{" "}
                              </li>
                              <li>
                                <span className="fa-li">
                                  <i className="fa fa-check"></i>
                              ...
                              <li>
                                ...
                                {plan["Post Limitation"]} post
                              </li>
                              <li>
                                <span className="fa-li">
                                  <i className="fa fa-check"></i>
                                </span>
                                {plan["Schedule and Publish"] != true
                                  ? ""
                                  : "manage and schedule posts"}
                              </li>
                              {plan["Content Curation"] == true ? (
                                <li>
                                  <span className="fa-li">
                                    <i className="fa fa-check"></i>
                                  </span>
                              ...
                              <li>
                                ...
                                {plan["Social Listening & Monitoring"]} monitor
                                activity
                              </li>
                              <li>
                                <span className="fa-li">
                              ...
                              {plan["Create and Manage Draft Posts"] == true ? (
                                <li>
                                  <span className="fa-li">
                                    <i className="fa fa-check"></i>
                                  </span>
                              ...
                              {plan["Team: Invite Additional Users"] == true ? (
                                <li>
                                  <span className="fa-li">
                                    <i className="fa fa-check"></i>
                                  </span>
                                  Team: Invite Additional Users
                                </li>
                              ) : (
                                ""
                              )}
                              {plan["Approval Workflow"] == true ? (
                                <li>
                                  <span className="fa-li">
                                    <i className="fa fa-check"></i>
                                  </span>
                            ...
                            {!profile.subscription.activeSubscription ? (
                              <button
                                className={`btn billing-btn active`}
                                onClick={() => {
                                  this.setState({ selectedPlan: plan });
                                  this.onPlanClick(plan["Name"].toLowerCase());
                                }}
                              >
                                Sign up
                              </button>
                            ...
                            plan["Monthly"] >
                              allPlans.find(
                                (item) =>
                                  item["Name"] ==
                                  roleBilling.charAt(0).toUpperCase() +
                                    roleBilling.slice(1)
                              )["Monthly"] ? (
                              <button
            ...
            {!payInfoEdit &&
              this.props.profile.subscription.activeSubscription && (
                <div className="payment-info">
                  <div className="row">
                    ...
                    <div className="col-md-4 col-sm-3">
                      <button
                        className="payment-info-edit"
                        onClick={() => this.onPayInfoEdit()}
                      >
                        Edit
                      </button>
                    </div>
              ...
              <div className="payment-detail">
                ...
                <div className="card-inputs form-field row">
                  <div className="col-12 col-md-6">
                    <input
                      className={"form-control whiteBg " + validClaas}
...
const mapStateToProps = (state) => {
  return {
    ...
    freeTrialEnded:
      state.profile.remain_date <= 0 &&
      state.profile.subscription.activeSubscription === false,
  };
};
const mapDispatchToProps = (dispatch) => ({
  startSetProfile: () => dispatch(startSetProfile()),
  startAddTwitterChannel: (accessToken, accessTokenSecret) =>
    dispatch(startAddTwitterChannel(accessToken, accessTokenSecret)),

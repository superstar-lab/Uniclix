...
class UpdateTeamMember extends React.Component {
  state = {
    ...
    publishChannels:
      !!this.props.member && this.props.member.assignedChannels.length > 0
        ? this.props.member.assignedChannels
        : this.props.channels,
    assignedChannels:
      !!this.props.member && this.props.member.assignedChannels.length > 0
        ? this.props.member.assignedChannels
  ...
  onChannelSelectionChange = (obj) => {
    const selectedPinterestChannel =
      !obj.selected && obj.type == "pinterest" ? obj : false;
  ...
  setAssignedChannels = () => {
    const assignedChannels = this.state.publishChannels
      .filter((channel) => channel.selected === 1)
      .map((channel) => ({
        ...channel,
        permissionLevel: channel.permissionLevel
          ? channel.permissionLevel
          : "member",
      }));
    this.setState(() => ({
      assignedChannels,
  ...
  render() {
    return (
      <div className="main-modal-style">
        <Modal
          isOpen={this.state.selectChannelsModal}
          closeTimeoutMS={300}
          ariaHideApp={false}
        ...
        <div className="modal-body">
            ...
            <div className="form-group">
                  ...
                  <div className="col-12 col-md-12 form-field m-sm" key={index}>
                    <div className="input-group">
                      <input
                        ...
                        onClick={this.toggleSelectChannelsModal}
                        readOnly
                        id={`channel-${channel.id}`}
                        placeholder="Name of the social account"
                      />
                      <select
                        id={`permission-${channel.id}`}
                        onChange={(e) => this.setPermissionLevel(e, channel.id)}
                        value={channel.permissionLevel}
                        className=""
                      >
              ...
              <div className="mb20">
                <div className="accounts-container__content__wrapper__footer">
                  <button
                    onClick={this.toggleSelectChannelsModal}
                    className="add-channel-plus-btn2"
                  >
                    <i className="fa fa-plus"></i>
                  </button>
                  <span className="left-side-label">
          ...
          <div className="modal-footer">
            <div>
              <button
                className="magento-btn small-btn modal-btn pull-right"
              ...
              <button
                ...
                className="btn btn-link pull-right modal-btn2 "
              >
                Cancel
              </button>
            </div>

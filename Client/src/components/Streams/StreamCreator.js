...
class StreamCreator extends React.Component {
  state = {
    ...
    searchTerm: "",
    verticalDisplay: this.props.verticalDisplay
      ? this.props.verticalDisplay
      : false,
  ...
  render() {
    return this.state.loading ? (
      <Loader />
    ) : isEmptyObject(this.state.selectedAccount) ||
      ...
      <div
        className={`streams-default-container ${
          this.state.verticalDisplay && "streams-default-container-new"
        }`}
      >
        <Modal
          isOpen={!!this.state.searchModal}
          ariaHideApp={false}
          className="stream-type-modal search-modal"
        >
          <div>
            <input
              type="text"
              ...
              placeholder="Example: coca cola or #fashion"
            />
            <button
              onClick={this.toggleSearchModal}
              className="publish-btn-group gradient-background-teal-blue link-cursor"
            >
              Done
            </button>
          </div>
        ...
        >
          <div>
            <AutoCompleteSearch
              ...
              channelId={this.state.selectedAccount.id}
              setSelected={this.setAutoCompleteSelected}
            />
            <button
              onClick={this.toggleAutoCompleteSearchModal}
              className="publish-btn-group autocomplete-done gradient-background-teal-blue link-cursor"
            >
              Done
            </button>
          </div>
        ...
        <div className="account-selection">
          <Select
            ...
            options={this.props.channels.map((channel) => {
              return {
                label: <ProfileChannel channel={channel} />,
                value: channel.name,
                type: channel.type,
                id: channel.id,
              };
        ...
        <div
          className={`streams-default ${
            this.state.verticalDisplay && "streams-default-new"
          }`}
        >
          {!this.state.defaultItem ? (
            streamTypes[this.state.selectedAccount.type].map((item, index) => (
              <div
                ...
                onClick={(e) => this.handleTypeClick(item)}
              >
            ...
            <button
              className="magento-btn"
              onClick={() => this.handleTypeClick(this.state.defaultItem)}
            >
              Create Stream
            </button>
          )}
        ...
        {typeof this.props.close !== "undefined" && (
          <div className="txt-center p10">
            <p onClick={this.props.close} className="link-cursor">
              Cancel
            </p>
          </div>
        )}

...
class StreamFeed extends React.Component {
  ...
  componentDidUpdate() {
    if (
      typeof this.props.streamItem !== "undefined" &&
      this.props.refreshId === this.props.streamItem.id
    ) {
      this.fetchFeed();
  ...
  fetchFeed = () => {
    ...
    getStreamFeed(
      streamItem.type,
      streamItem.network,
      streamItem.channel_id,
      streamItem.search_query,
      ""
    )
      .then((response) => {
        const items =
        ...
        if (
          typeof response["paging"] !== "undefined" &&
    ...
    getStreamFeed(
      ...
      .then((response) => {
        ...
        if (
          ...
          items[0].id === this.state.items[0].id
        ) {
          items = [];
        }
  ...
  render() {
    const { streamItem, channel, refreshRate, reload, selectedTab } =
      this.props;
    const { imageViewer, imageIndex, images } = this.state;
    return (
      <div
        ref={(ref) => (this.scrollParentRef = ref)}
        className="stream-feed scrollbar"
      >
        <ReactInterval
          timeout={refreshRate * 60000}
          enabled={true}
          ...
          {this.state.items.length ? (
            ...
            <div className="container-nodata">
              <div>
                ...
                <span>
                  {this.state.error ? this.state.error : "No data found."}
                </span>
                {this.state.errorStatus === 401 && (
                  <div>
                    <NavLink to={`/accounts/${channel.type}`}>
                      Reconnect
                    </NavLink>
                  </div>
                )}

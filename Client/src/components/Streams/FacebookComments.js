...
class FacebookComments extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.post.comments !== post.comments && this.state.showComments) {
      ...
      getComments(post.id, channel.id)
        .then((response) => {
          this.setState(() => ({
            comments: response,
  ...
  render() {
    ...
    return (
      <div className="comments">
        {post.comments.data.length > 0 && !this.state.emptyList && (
          <a
        ...
        {this.state.showComments &&
          this.state.comments.map((comment, index) => (
            <div className="comment" key={comment.id}>
                ...
                <div>
                  ...
                  {typeof comment.replies === "undefined" && (
                    <div className="view-replies">
                      <a
                        href="javascript:void(0)"
                        onClick={() => this.getReplies(comment.id)}
                      >
                        View replies
                      </a>
                    </div>
              ...
              {this.state.loadingReplies === comment.id && (
                <div className="loader-container">
                  <AvatarWithText />
                </div>
              )}
              ...
              {typeof comment.replies !== "undefined" &&
                comment.replies.map((reply) => (
                  <div
                    key={reply.id}
                    className="comments__commenter comments__reply"
                  >
                    <div className="commenter__profile">
                      <div className="commenter__profile__pic">

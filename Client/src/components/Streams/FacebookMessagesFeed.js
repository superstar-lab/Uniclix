...
class FacebookMessagesFeed extends React.Component {
  ...
  toggleConversation = (message = "") => {
    this.setState(
      ...
      () => {
        if (message == "success") {
          toastContainer.success("Message posted.", "Success", {
            closeButton: true,
          });
        }

        if (message == "error") {
          toastContainer.error("Something went wrong.", "Error", {
            closeButton: true,
          });
        }
      }
  ...
  render() {
    try {
      ...
      const text = feedItem.messages.data[0].message
      ...
      return (
        <div>
          ...
          <div
            className={`stream-feed-container fb-conversations`}
            onClick={this.toggleConversation}
          >
            <div className="post-info">
              <img
                src={
                  channel.name === feedItem.messages.data[0].from.name
                    ? channel.avatar
              ...
              <div className="post-info-item">
                <a href="#" className="username">
                  <strong>{feedItem.messages.data[0].from.name}</strong>
                </a>
                <div className="post-date">
                  {toHumanTime(feedItem.messages.data[0].created_time)}
                </div>
              </div>
            </div>

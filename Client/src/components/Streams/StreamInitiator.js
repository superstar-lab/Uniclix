...
class StreamInitiator extends React.Component {
  ...
  initialValue = () => {
    ...
    if (socialMediasSelectorOptions.length <= 0) return;
  ...
  onAccountChange = (selectedAccountId) => {
    this.props.onChangeAccount(selectedAccountId);
    let selectedAccount = this.props.channels.find(
      (item) => item.id === selectedAccountId
    );
    if (selectedAccount) {
      this.setState({ selectedAccount: selectedAccount });
  ...
  render() {
    const { selectedSocial, selectedAccountId, socialMediasSelectorOptions } =
      this.state;
    return (
      <div>
        <Modal

...
class FacebookActions extends React.Component {
  ...
  commentPost = () => {
    ...
    comment(feedItem.id, channel.id, content, image, video)
      .then((response) => {
        this.setState(() => ({
          comment: false,
        }));
  ...
  render() {
    ...
    const likesCount =
      feedItem.likes.summary.total_count > 0
        ? abbrNum(feedItem.likes.summary.total_count)
        : "";
    const commentsCount =
      feedItem.comments.summary.total_count > 0
        ? abbrNum(feedItem.comments.summary.total_count)
        : "";
      ...
      <div className="fb-actions-container">
        ...
        <div className="stream-action-icons">
          <img
            ...
            onClick={this.togglePostBox}
            src="images/monitor-icons/back-small.svg"
          ></img>
          <span>
            <img
              className="action-facebook-icon-button"
              onClick={() => this.toggleComment()}
              src="images/monitor-icons/conversations.svg"
            />
            <span className={`status-counter ${commentPost}`}>
              {" "}
              {commentsCount}
            </span>
          </span>
          <span>
            <img
              ...
              src="images/monitor-icons/heart-contact-small.svg"
            />
            <span className={`status-counter ${likedPost} `}>
              {" "}
              {likesCount}
            </span>
          </span>
              ...
              <div className="t-reply-container">
                ...
                <div className="t-reply-footer">
                  ...
                  <div className="t-reply-actions">
                    <button onClick={this.toggleComment} className="cancelBtn">
                      Cancel
                    </button>
                    {this.state.letterCount < 1 ||
                    this.state.letterCount > 250 ? (
                      <button className="doneBtn disabled-btn">Send</button>
                    ) : !this.state.loading ? (
                      <button onClick={this.commentPost} className="doneBtn">
                        Send
                      </button>
                    ) : (

...
const TwitterDefaultFeed = ({
  ...
  try {
    ...
    let media =
      typeof feedItem.extended_entities !== "undefined" &&
      typeof feedItem.extended_entities.media !== "undefined"
        ? feedItem.extended_entities.media
        : [];
    media = media.map((file) => {
      const source =
        file.type === "video" &&
        typeof file.video_info.variants !== "undefined" &&
        file.video_info.variants.length
          ? file.video_info.variants[0].url
          : "";
...
const TwitterFollowersFeed = ({
  ...
  try {
    const text =
      ...
      typeof feedItem.status["text"] !== "undefined"
        ? feedItem.status["text"]
        : "";
    const date =
      typeof feedItem.status !== "undefined" &&
      typeof feedItem.status["created_at"] !== "undefined"
        ? feedItem.status["created_at"]
        : "";
    const username = feedItem.screen_name;
    const accountId = feedItem.id_str;
    const profileImg = feedItem.profile_image_url_https;
    const statusId =
      typeof feedItem.status !== "undefined" ? feedItem.status.id_str : "";
    const networkType = "twitter";
    let media =
      typeof feedItem.status !== "undefined" &&
      typeof feedItem.status.extended_entities !== "undefined" &&
      typeof feedItem.status.extended_entities.media !== "undefined"
        ? feedItem.status.extended_entities.media
        : [];
    media = media.map((file) => {
      const source =
        file.type === "video" &&
...
const ScheduledFeed = ({
  ...
  try {
    ...
    let media =
      typeof feedItem.payload.images !== "undefined" &&
      feedItem.payload.images.length
        ? feedItem.payload.images
        : [];
...
const FacebookPostsFeed = ({
  ...
  try {
    ...
    const attachments =
      typeof feedItem.attachments !== "undefined"
        ? feedItem.attachments.data
        : [];
    const subAttachments =
      attachments.length && typeof attachments[0].subattachments !== "undefined"
        ? typeof attachments[0].subattachments.data !== "undefined"
          ? attachments[0].subattachments.data
          : []
        : [];
    const networkType = "facebook";

    let mainMedia = attachments
      .map((item) => {

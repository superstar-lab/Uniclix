...
class TwitterReplies extends React.Component {
  ...
  componentDidMount() {
    ...
    if (!keyword) {
      ...
      getStreamFeed("search", "twitter", channel.id, keyword, "")
        .then((response) => {
          const items =
    ...
    return (
      <div className="body-wrap">
        <div className="t-reply-container">
          ...
          {keyword && (
            <div className="text-center m10">
              <button className="btn btn-primary" onClick={this.submitStream}>
                Save as new stream
              </button>
            </div>
          )}
...
const TwitterStatusReply = ({ reply, parentPostData, reload }) => {
  ...
  let media =
    typeof reply.extended_entities !== "undefined" &&
    typeof reply.extended_entities.media !== "undefined"
      ? reply.extended_entities.media
      : [];

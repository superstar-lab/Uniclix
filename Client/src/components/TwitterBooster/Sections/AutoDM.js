...
import {
  ...
  saveAutoMessages,
  getAutoMessages,
} from "../../../requests/twitter/channels";
...
class AutoDM extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      this.props.selectedChannel !== prevProps.selectedChannel ||
  ...
  selectMessage = (val) => {
    if (!!this.state.isADMactive)
      this.setState({
        keyword: val,
        replaceKeyword: this.state.replaceKeyword + 1,
      });
  };
  activateDm = (e) => {
    ...
    return activateADM(channelId, status)
      .then((response) => {
        ...
        toastContainer.success(`Auto DM ${message} successfully.`, "Success", {
          closeButton: true,
        });
        this.fetchData();
        return Promise.resolve(response);
      ...
      return saveAutoMessages(channelId, status)
        .then((response) => {
          toastContainer.success(
            `Auto DM response set successfully.`,
            "Success",
            { closeButton: true }
          );
          this.setState({
            ...
            <div className="section-header">
              <div className="section-header__first-row">
                <h2>
                  Auto DM
                  <label
                    className={`switch round ${!!isADMactive ? "checked" : ""}`}
                  >
                    <input
                      type="checkbox"
                      defaultChecked={!!isADMactive ? "checked" : ""}
              ...
              <div className="section-header__second-row">
                <p>
                  This is your Auto direct message, you can choose and edit a
                  predefined one or start writing your own.
                </p>
              </div>
            </div>
          </div>
                ...
                <div className="tab-head">
                  <div
                    className={`tab-nav-item ${
                      isTabActive == "predefined" ? "active" : ""
                    }`}
                  >
                    <button
                      href="#personal-info"
                      onClick={() => this.ChangeTab("predefined")}
                    >
                      Predefined
                    </button>
                  </div>
                  <div
                    className={`tab-nav-item ${
                      isTabActive == "created-by-me" ? "active" : ""
                    }`}
                  >
                    <button
                      href="#company-info"
                      onClick={() => this.ChangeTab("created-by-me")}
                    >
                      Created by me
                    </button>
                  </div>
                    ...
                    <ul className="list-items">
                      {predefinedMessages.map((predMessage, index) => (
                        <li className="list-item" key={index}>
                          {predMessage}
                          <button
                            className="blue-txt-btn add-message"
                            onClick={() => this.selectMessage(`${predMessage}`)}
                          >
                            Select
                          </button>
                        </li>

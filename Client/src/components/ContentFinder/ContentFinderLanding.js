...
class ContentFinderLanding extends React.Component {
  ...
  setLoadingState = (isLoading, showTopics = true) =>
    this.setState({
      isLoading,
      showTopics,
    });
  ...
  setFilterTopic = (topic) => {
    this.setState({
      filterTopics: [...this.state.filterTopics, topic.toUpperCase()],
    });
  };
  ...
  render() {
    ...
    const topicsAddedBYou = selectedTopics.filter(
      (topic) => TRENDING_TOPICS.indexOf(topic) === -1
    );
    ...
    return (
      <div className="content-finder">
        ...
        {showTopics && (
          <React.Fragment>
            <AddedByYouSection
              topicsAddedByYou={topicsAddedBYou}

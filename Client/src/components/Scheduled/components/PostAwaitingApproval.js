...
class PostAwaittingApproval extends React.Component {
    ...
    approvePost(post_id)
      ...
      .catch((error) => {
        ...
        notification.error({
          message: "Error :(",
          description:
            "Something went wrong and the post could not get approved. Please try again later.",
        });
        this.setState({ isLoading: false });
      });
  };
  ...
  render() {
    const {
      ...
      content,
      category,
      payload: { scheduled, images },
    } = this.props;
    const { confirmationIsOpen, isLoading, modalTitle, modalOnOkCallback } =
      this.state;
    ...
    return (
      <div className="post-awaiting" style={{ borderColor: category.color }}>
        ...
        <div className="channels">{this.renderChannels()}</div>
        <div className="publish-date">
          {moment(scheduled.publishUTCDateTime)
            .tz(timezone)

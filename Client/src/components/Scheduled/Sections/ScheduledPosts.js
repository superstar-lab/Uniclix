...
class ScheduledPosts extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      prevState.startDate.format("YYYY-MM-DD") !==
  ...
  fetchPosts = () => {
    ...
    try {
      scheduledPosts(
        cloneStart.format("YYYY-MM-DD"),
        cloneEnd.format("YYYY-MM-DD")
      )
        .then((response) => {
  ...
  fetchMoreData = () => {
    ...
    let tmpItems = items;
    let tmpPage = page;
    const cloneStart = moment()
        .tz(timezone)
        .add(tmpPage * cntDate, "days")
        .subtract(1, "days")
        .format("YYYY-MM-DD"),
      cloneEnd = moment()
        .tz(timezone)
        .add((tmpPage + 1) * cntDate, "days")
        .format("YYYY-MM-DD");
        ...
        .then((response) => {
          ...
          for (let i = tmpPage * cntDate; i < (tmpPage + 1) * cntDate; i++) {
            ...
            if (date.day() === 0) {
              ...
              settingTimes = JSON.parse(
                JSON.stringify(schedulingTimes[date.day() - 1])
              );
            }
            for (let j = 0; j < settingTimes.length; j++) {
              for (let k = 0; k < posts.length; k++) {
                if (
                  posts[k].is_best === 1 &&
                  posts[k].payload.scheduled.publishDateTime.replace(
                    "T",
                    " "
                  ) ===
                    date.format("YYYY-MM-DD") + " " + settingTimes[j].time
                ) {
                  posts[k].payload.scheduled.publishDateTime = posts[
                    k
                  ].payload.scheduled.publishDateTime.replace("T", " ");
            ...
            for (let j = 0; j < posts.length; j++) {
              if (
                posts[j].is_best !== 1 &&
                posts[j].payload.scheduled.publishDateTime.substring(0, 10) ===
                  date.format("YYYY-MM-DD")
              ) {
                posts[j].payload.scheduled.publishDateTime = posts[
                  j
                ].payload.scheduled.publishDateTime.replace("T", " ");
            ...
            settingTimes.sort((a, b) => {
              const timeA =
                a.time === undefined
                  ? a.payload.scheduled.publishDateTime.substr(11)
                  : a.time;
              const timeB =
                b.time === undefined
                  ? b.payload.scheduled.publishDateTime.substr(11)
                  : b.time;
            ...
            tmpItems.push({
              ...
              monthNames: monthNames[date.month()],
              date: date.date(),
  ...
  render() {
    ...
    return (
      <div className="calendar-events">
        ...
        <Compose
          onPost={() => {
            this.onResetPage(), this.fetchPosts(), this.fetchMoreData();
          }}
        />
      </div>
    );
